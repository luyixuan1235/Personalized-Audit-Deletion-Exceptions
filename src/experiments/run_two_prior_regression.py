...
X=torch.tensor([[r["ch"],r["cc"],r["ch"]*r["cc"],r["lsup"]] for r in rows],dtype=torch.float32)
FE=torch.tensor([di[r["dtype"]] for r in rows])
Y=torch.tensor([float(r["ov"]) for r in rows])
torch.manual_seed(42)
w=torch.zeros(4,requires_grad=True); fe=torch.zeros(len(dts),requires_grad=True); b0=torch.zeros(1,requires_grad=True)
...
def fit(idx):
    ...
    ww=torch.zeros(4,requires_grad=True); ff=torch.zeros(len(dts),requires_grad=True); bb=torch.zeros(1,requires_grad=True)
...
names=["contra_habit","contra_crowd","interaction","log_support"]
out={n:{ "coef":float(w.detach().numpy()[i]),
        "ci95":[float(np.percentile(boots[:,i],2.5)),float(np.percentile(boots[:,i],97.5))]}
     for i,n in enumerate(names)}
out["n"]=len(rows); out["note"]="logit(override) ~ contra_habit + contra_crowd + interaction + log1p(user-conjunction support) + data-type FE; FM/bce, Wu, 5-fold; decision bootstrap 100x"
print(json.dumps(out,indent=1))
json.dump(out, open(os.path.join(os.environ.get("PERM_RESULTS_DIR","../results"),"table57_regression.json"),"w"), indent=1)
